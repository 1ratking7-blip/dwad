...
## Настройка аналитики
GA4 подключён (`G-0RLZ7LGT08`) — события уходят в реальный GA4-поток. Yandex Metrica пока не настроена: раскомментируйте соответствующий блок в `index.html` и подставьте номер счётчика (`YM_COUNTER_ID`), когда он появится.
