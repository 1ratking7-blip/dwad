...
- **GitHub Actions `telegram-daily-post.yml` (cron 12:00 UTC = 19:00 ICT) ненадёжен**: за 36+ часов
  (26-27 июля) отработал только один раз (проверено `api.github.com/.../runs` — 1 run, 26 июля,
  event=schedule, success; на 27 июля run вообще не создался). Пост за 27 июля ("Футбол — 16 лет")
  опубликован вручную 2026-07-27 (`python Scripts/post_scheduled.py 2026-07-27`, message_id 298).
  Известное ограничение GitHub (cron-триггеры могут пропускаться при нагрузке платформы), не баг
  в коде скрипта. Пост за 28 июля (19:00 ICT) ещё не наступил на момент этой записи — стоит
  проверить вручную вечером, сработал ли автопостинг, и повторить ручную публикацию при
  необходимости, пока не решено ставить ли резервный механизм (напр. ScheduleWakeup-проверка).
