...
const LINK = "https://bcall-loop.bcgame-bet.com/dispatch-v6?i=zhelezo&p=/login/regist";
...
Команда ZHELEZO`
    },
    {
        ...
        body: `Привет! Как успехи с Lucky Spin? Надеемся, тебе выпал сочный куш!
...
function generateContent() {
    ...
    for (const [platform, tpls] of Object.entries(TEMPLATES)) {
        tpls.forEach((tpl, i) => {
            ...
            const fileName = path.join(outputDir, `${platform}_var_${i + 1}.txt`);
    ...
    const divider = '\n\n' + '─'.repeat(40) + '\n\n';
    const telegramCalendar = CALENDAR.map(d => `День ${d.day} — ${d.topic}\n\n${d.telegram}`).join(divider);
    const twitterCalendar = CALENDAR.map(d => `Day ${d.day} — ${d.topic}\n\n${d.twitter}`).join(divider);

    fs.writeFileSync(path.join(outputDir, 'telegram_30day_calendar.txt'), telegramCalendar, 'utf8');
    fs.writeFileSync(path.join(outputDir, 'twitter_30day_calendar.txt'), twitterCalendar, 'utf8');
    total += 2;

    const emailChain = EMAILS.map((e, i) => `Письмо ${i + 1} — ${e.trigger}\nТема: ${e.subject}\n\n${e.body}`).join(divider);
    fs.writeFileSync(path.join(outputDir, 'email_nurture_sequence.txt'), emailChain, 'utf8');
    total += 1;

    console.log(`✅ Успешно сгенерировано ${total} файлов контента в ${outputDir}`);
}
