...
1. **Домен** — `zhelezo.io` проверен через RDAP и был свободен на момент проверки; сейчас
   сайт живёт на бесплатном `zhelezo.pages.dev`. Подключение своего домена — через
   Cloudflare DNS, требует регистрации домена у любого регистратора и добавления его
   в тот же Cloudflare-аккаунт.
